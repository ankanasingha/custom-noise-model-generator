...
def create_noise_model(ecr_error_rates, qubit_noise_params):
    ...
    for qubit, params in qubit_noise_params.items():
        ...
        t2 = params['t2']
        if t2 > 2 * t1:
            t2 = 2 * t1  # Correct T2 if it exceeds 2 * T1
        thermal_error = thermal_relaxation_error(t1, t2, 660e-9)
    ...
    # ECR gate errors for specific qubit pairs
...
def pre_optimise_version_between_real_and_simulated_count_in_4q_excluding_crosstalk():
...
if __name__ == "__main__":
    pre_optimise_version_between_real_and_simulated_count_in_4q_excluding_crosstalk()
